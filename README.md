...
`tools/list` at `https://gateway.pipeworx.io/data-vermont/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/data_vermont_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"budget"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/data_vermont_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "data-vermont": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-data-vermont"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-data-vermont
```
